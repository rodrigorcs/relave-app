...
export const serviceBundlesActions = {
  ...
  getAllWithDetails: async (): Promise<IServiceBundleWithDetails[]> => {
    ...
    const serviceBundleWithDetails = serviceBundles.map((serviceBundle) => {
      const exclusiveServicesInBundle = services.filter((service) =>
        serviceBundle.exclusiveServices.includes(service.id),
      )
      const allServicesInBundle = services.filter((service) =>
        serviceBundle.allServices.includes(service.id),
      )
      return {
        ...serviceBundle,
